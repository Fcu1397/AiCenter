<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <div class="bg-white p-8 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">透明度查詢平台</h1>
    ...
    >
      <div v-if="selectedProduct" class="space-y-6">
        ...
        <div>
          <h3 class="font-bold text-lg mb-4 border-b pb-2">九大透明性原則揭露</h3>
          <div v-for="rule in MOCK_TRANSPARENCY" :key="rule.principle_no" class="mb-6">
            <h4 class="font-semibold text-blue-800 mb-2">
              {{ rule.principle_no }}. {{ rule.principle_name }}
            </h4>
            <div class="bg-gray-50 p-4 rounded text-gray-700 text-sm leading-relaxed">
              {{ rule.content }}
            </div>
          </div>
        </div>
